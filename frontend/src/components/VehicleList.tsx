...
export const VehicleList: React.FC = () => {
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 bg-slate-950 space-y-6 md:space-y-8 flex-1 overflow-y-auto relative">
      {/* Title Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 select-none">
        <div>
          <h2 className="text-2xl font-black text-white tracking-wide font-display">Active Yard Stock Console</h2>
          <p className="text-xs text-slate-400 font-semibold mt-0.5">Search vehicle slots, track status workflows, manage dynamic billing and approvals</p>
        </div>
      </div>

      {/* Filters & Search Console */}
      <div className="bg-slate-900/60 backdrop-blur-md p-5 rounded-2xl border border-slate-800/80 shadow-xl flex flex-col md:flex-row items-center justify-between gap-4">
        {/* Search */}
        <div className="relative w-full md:max-w-md select-none">
          ...
          <input
            ...
            className="w-full text-white bg-slate-950 pl-10 pr-4 py-2.5 rounded-xl border border-slate-800 focus:outline-none focus:border-indigo-500 text-xs font-semibold placeholder-slate-500"
          />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto select-none">
          <div className="flex items-center space-x-1.5 bg-slate-950 px-3 py-2 rounded-xl border border-slate-800">
            ...
            <span className="text-[10px] font-extrabold text-slate-300 uppercase tracking-wider">Filters</span>
          </div>

          <select
            value={typeFilter}
            onChange={(e) => setTypeFilter(e.target.value)}
            className="text-xs font-bold text-white bg-slate-950 border border-slate-800 rounded-xl px-3 py-2.5 focus:outline-none focus:border-indigo-500"
          >
      ...
      {/* Desktop Stock Table */}
      <div className="hidden md:block bg-slate-900/60 backdrop-blur-md rounded-2xl border border-slate-800/80 shadow-xl overflow-hidden select-none">
            ...
            <tbody className="divide-y divide-slate-800/60 text-slate-300 font-medium">
              {loading ? (
                ...
                vehicles.map((v) => (
                  ...
                  >
                    <td className="p-4 font-black text-white uppercase tracking-wider">{v.vehicleNumber}</td>
                    <td className="p-4 text-slate-200">{v.brand || 'N/A'} {v.model || ''}</td>
                    <td className="p-4">
                      <div className="text-white font-semibold">{v.bankName}</div>
                      <div className="text-[10px] text-slate-400 font-medium mt-0.5">{v.repoAgency || 'Swift Agency'}</div>
                    </td>
                    <td className="p-4 text-slate-400">{new Date(v.entryDate).toLocaleDateString('en-IN')}</td>
                    <td className="p-4">
                      <span
                        className={`px-2.5 py-0.5 rounded-full font-extrabold text-[9px] ${
                          v.yardStatus === 'KACHHA'
                    ...
                    <td className="p-4">
                      <span className="font-bold text-slate-300 bg-slate-950 border border-slate-800 px-2.5 py-1 rounded-lg">
        ...
        {/* Pagination Controls */}
        <div className="bg-slate-950 border-t border-slate-800 p-4 flex items-center justify-between select-none">
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              disabled={page === 1}
              className="px-3 py-1.5 rounded-xl border border-slate-800 bg-slate-900 text-xs font-bold text-slate-300 disabled:opacity-50 hover:bg-slate-800 transition-colors"
            >
              Previous
            </button>
